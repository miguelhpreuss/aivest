...
def CMA(data):
    ...
    return data["Close"].expanding().mean()
...
def get_detailed_info(data):
    response = urllib.request.urlopen(f'https://query2.finance.yahoo.com/v1/finance/search?q={data}')
    content = response.read()
    return pd.DataFrame(json.loads(content.decode('utf8'))['quotes'])

    
def MF(data, period=14):
    """median filter"""
    return data.rolling(period, win_type='gaussian').mean(std=data['Close'].std().mean())
